...
export function EntryCard({ entry, rank }: EntryCardProps) {
  ...
  const imageSource = entry.photoUrl
    ? { uri: entry.photoUrl }
    : images.entryCardFallback;

  return (
    <View className="rounded-bestlist-xl bg-white px-5 py-4.5 shadow-card">
        ...
        <View className="flex-row items-start justify-between gap-3">
          <View className="flex-1 pr-2">
            ...
            <View className="mt-2 flex-row items-start gap-3 ">
              <Image
                source={imageSource}
                className="h-19 w-19 rounded-bestlist-sm"
              />
              <View className="min-w-0 flex-1 mt-1">
                <Text
