...
import {
  ...
  getProfileInitial,
  mapProfileRow,
  prepareAppleNameProfileUpdate,
  prepareSetupHandleProfileUpdate,
  summarizeProfileCategories,
} from "./profile-data.ts";

test("prepares a trimmed Apple full name for the profiles table", () => {
  assert.deepEqual(prepareAppleNameProfileUpdate("  Gina   Pham  "), {
    full_name: "Gina Pham",
  });
});

test("does not prepare an empty Apple full name", () => {
  assert.equal(prepareAppleNameProfileUpdate("   "), null);
});
